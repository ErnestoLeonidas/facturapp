...
r.post('/', (req, res) => {
  ...
  const { items, monto_neto_clp, monto_iva_clp, monto_total_clp } = calcularTotales(b.items, b.empresa_emisora, b.moneda_base, b.uf_valor);
  ...
  const ins = db.transaction(() => {
    ...
    (b.cps || []).forEach((c, i) => {
      ...
      if (cpRow) db.prepare('INSERT INTO solicitud_cp (id, solicitud_id, cp_id, monto_clp, orden) VALUES (?,?,?,?,?)')
        .run(uuidv4(), id, cpRow.id, c.monto_clp||0, i);
    });
  ...
  ok(res, hydrateOne(db.prepare('SELECT * FROM solicitud_factura WHERE id = ?').get(id)), 201);
});

r.get('/:id', (req, res) => {
  const row = db.prepare('SELECT * FROM solicitud_factura WHERE id = ? OR folio = ?').get(req.params.id, req.params.id);
  if (!row) return notFound(res);
  ok(res, hydrateOne(row));
});

r.patch('/:id', (req, res) => {
  const sol = db.prepare('SELECT * FROM solicitud_factura WHERE id = ?').get(req.params.id);
  ...
  const b = req.body;
  const { items, monto_neto_clp, monto_iva_clp, monto_total_clp } = calcularTotales(
    b.items !== undefined ? b.items : db.prepare('SELECT * FROM solicitud_item WHERE solicitud_id=?').all(sol.id),
  ...
  if (b.cps !== undefined) {
    ...
    b.cps.forEach((c, i) => {
      const cpRow = db.prepare('SELECT id FROM cp WHERE codigo = ?').get(c.cp_codigo || c.cp_id);
      if (cpRow) db.prepare('INSERT INTO solicitud_cp (id, solicitud_id, cp_id, monto_clp, orden) VALUES (?,?,?,?,?)')
        .run(uuidv4(), req.params.id, cpRow.id, c.monto_clp||0, i);
    });
  }
  ...
  ok(res, hydrateOne(db.prepare('SELECT * FROM solicitud_factura WHERE id = ?').get(req.params.id)));
});
...
r.post('/:id/duplicar', (req, res) => {
  const orig = db.prepare('SELECT * FROM solicitud_factura WHERE id = ?').get(req.params.id);
  ...
  ok(res, hydrateOne(db.prepare('SELECT * FROM solicitud_factura WHERE id = ?').get(newId)), 201);
});

r.get('/:id/historial', (req, res) => {
  const row = db.prepare('SELECT id FROM solicitud_factura WHERE id = ?').get(req.params.id);
  if (!row) return notFound(res);
